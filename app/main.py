...
from app.models import (
    PredictRequest, PredictResponse, TopSkillsResponse, SkillItem, HealthResponse,
    SkillRecommendationResponse, SkillRecommendationItem
)
...
# ── FastAPI app ───────────────────────────────────────────────────────────────
app = FastAPI(
    title="SkillScavenge API",
    description="Real-time job market analytics and salary prediction API for SkillScavenge (formerly SkillPulse). "
                "Built on top of multi-source job postings and an XGBoost regressor.",
    version="1.0.0",
    contact={"name": "Harshdeep Singh", "email": "Harshdeep@skillscavenge.io"},
...
@app.get("/skills/recommend/{skill_name}", response_model=SkillRecommendationResponse, tags=["Skills"])
def recommend_skills(skill_name: str, limit: int = Query(8, ge=1, le=20)):
    """
    Returns top N semantically similar skills using sentence-transformers cosine similarity.
    """
    recs = recommend_similar_skills(skill_name, top_n=limit)
    return SkillRecommendationResponse(
        target_skill=skill_name,
        total_returned=len(recs),
        recommendations=[SkillRecommendationItem(**r) for r in recs]
    )
...
def predict_salary(payload: PredictRequest):
    ...
    shap_info = compute_prediction_shap(model, feature_vector, FEATURE_NAMES, top_n=10)
    ...
    return PredictResponse(
        ...
        base_value_log=shap_info["base_value_log"],
        feature_contributions=shap_info["feature_contributions"],
    )
